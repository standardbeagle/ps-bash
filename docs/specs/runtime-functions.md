...
| yq | `InvokeBashYqCommand` | 4-follow-on | YAML query — the jq-style filter engine applied to YAML input. Paired follow-on with the F6 jq migration. The cmdlet drives flag parsing, file / pipeline collection, and dispatches YAML parse, jq filter, and output emission (JSON or YAML) to the surviving psm1 helpers via parameter-bound `InvokeCommand.InvokeScript` (AOT-safe — no `ScriptBlock` construction, no user-token concatenation, Directive 12). **Why the psm1 helpers stay:** there is no .NET YAML lib in stdlib, so `ConvertFrom-SimpleYaml` / `ConvertFrom-YamlValue` / `ConvertTo-SimpleYaml` cannot migrate without a third-party dependency; and the C# `JqEngine` operates on the `System.Text.Json` graph that `ConvertFrom-Json -AsHashtable` produced, not the `OrderedDictionary` graph that `ConvertFrom-SimpleYaml` produces — bridging the two graphs would duplicate the jq engine in C#. Keeping the psm1 helpers preserves byte-for-byte parity with the oracle without re-implementing either YAML or jq in C#. Flag surface: `-r` / `--raw-output` (raw string output), `-o FORMAT` / `--output-format FORMAT` (default `json`, also `yaml`). **One colliding flag** declared as an explicit value-bearing parameter with a single-letter name: `-o` prefix-collides with `-OutVariable` / `-OutBuffer` under the cmdlet binder (an `[Alias("o")]` would lose to the common-parameter prefix match), so the parameter is literally named `O`. The long form `--output-format` continues to flow through `Arguments` and is recovered post-parse. `-r` has no PowerShell common-parameter prefix overlap and stays in `Arguments`. File-input errors emit through `InvokeCommand.InvokeScript("param($m) Write-BashError -Message $m", message)`. Pipeline collection mirrors the oracle's `StringBuilder` + `Trim` slice. Output uses `BashRuntime.NewBashObject` (default `PsBash.TextOutput`). Replaces a 83-line psm1 function. Parity tests in `PsBash.Cmdlets.Tests/InvokeBashYqCommandTests.cs` cover field filter (quoted + `-r`), identity, `-o yaml` output, pipeline mode, nested filter, integer preservation, unicode, missing file, empty pipeline, `yq` alias resolution, `--help`, and a Directive-12 injection probe with `$(throw 'PWNED')` as the filter — 13 tests total |
