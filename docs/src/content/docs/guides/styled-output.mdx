...
Eleven sheets ship embedded in the cmdlet. Name them with `-Style <name>`:
...
| `av` | `psav` media + artifacts | `.video` cyan, `.audio` magenta, `.image` green; `.ok` green, `.failed` red+bold, `.planned` gray italic |
