...
internal static class FileSystemHelpers
{
    ...
    /// reasons they are pruned: (a) users almost never want hits inside them, and
    /// (b) walking into them is what tripped the host idle-timeout watchdog — the
    /// old <c>Directory.EnumerateFiles(root, "*", AllDirectories)</c> drained
    /// millions of <c>.git</c> objects (and every <c>bin</c>/<c>obj</c> build
    /// artifact) with no output for 120s before the first match. Pruning happens
    /// BEFORE descent, so their contents are never enumerated.
    /// <para>
    /// This deliberately diverges from GNU <c>grep -r</c>, which prunes nothing —
    /// a quality-of-life default. Set <see cref="NoIgnoreEnvVar"/> (or pass rg's
    /// <c>--no-ignore</c>) to restore the unfiltered walk and search these trees.
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// Environment variable that, when set to a truthy token
    /// (<c>1</c>/<c>true</c>/<c>yes</c>/<c>on</c>, case-insensitive), disables the
    /// default directory pruning (<see cref="DefaultPrunedDirectories"/>) for both
    /// <c>grep -r</c> and <c>rg</c> — i.e. search everything, matching GNU grep's
    /// unfiltered behavior. The runtime config knob for the filtering default.
    /// </summary>
    public const string NoIgnoreEnvVar = "PSBASH_SEARCH_NO_IGNORE";

    /// <summary>
    /// True when <see cref="NoIgnoreEnvVar"/> is set to a truthy value — default
    /// directory pruning is turned off. Self-contained truthy parse (the Cmdlets
    /// assembly does not reference <c>PsBash.Core.Runtime.EnvFlags</c>; the token
    /// set is kept identical on purpose).
    /// </summary>
    ...
    /// Prunes <see cref="DefaultPrunedDirectories"/> (unless <paramref
    /// name="includeIgnored"/>) and dot-directories / dot-files (unless <paramref
    /// name="includeHidden"/>) BEFORE descending, so their contents are never
    /// walked. Yields lazily in a deterministic pre-order DFS (directories and
    /// files sorted by ordinal name) so the caller streams matches as the tree is
    /// walked instead of after a full enumeration — this is what keeps the host
    /// from going silent long enough to trip its idle-timeout watchdog.
    /// Unreadable directories are skipped best-effort.
    /// </summary>
        ...
        string root, bool includeIgnored, bool includeHidden)
    {
        ...
        while (stack.Count > 0)
        {
            ...
            // Push in reverse ordinal order so the lexicographically-first
            // subdirectory pops first → stable pre-order DFS.
