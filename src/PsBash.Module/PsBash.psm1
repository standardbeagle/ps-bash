...
if ($script:__psbashCmdletsDll) {
    # -Scope Global (not just -Global on Import-Module — that makes them visible
    # globally but they still live in the binary submodule). Combined with -Force
    # to defeat any earlier function-stub registration.
    Import-Module $script:__psbashCmdletsDll -Global -Force -ErrorAction SilentlyContinue
}
